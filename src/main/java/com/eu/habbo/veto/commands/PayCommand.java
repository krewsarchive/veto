...
public class PayCommand extends Command
{
    ...
    public boolean handle(GameClient gameClient, String[] strings) throws Exception
    {
        if (strings.length < 4)
        {
            gameClient.getHabbo().whisper(Emulator.getTexts().getValue("veto.cmd_pay.incorrect.usage"));
            return true;
        }
        ...
        {
            gameClient.getHabbo().whisper(Emulator.getTexts().getValue("veto.cmd_pay.self"));
            return true;
        }
        ...
        if (!valid || amount <= 0)
        {
            gameClient.getHabbo().whisper(Emulator.getTexts().getValue("veto.cmd_pay.invalid_amount"));
            return true;
        }
        ...
        if (strings[3].equalsIgnoreCase("credits") || strings[3].equalsIgnoreCase("coins"))
        {
            ...
            {
                gameClient.getHabbo().whisper(Emulator.getTexts().getValue("veto.cmd_pay.not_enough").replace("%amount%", amount + "").replace("%type%", strings[3]).replace("%username%", habbo.getHabboInfo().getUsername()));
                return true;
            }
        }
        else
        {
            ...
            if (!found || type == -1)
            {
                gameClient.getHabbo().whisper(Emulator.getTexts().getValue("veto.cmd_pay.invalid_type").replace("%type%", strings[3]));
                return true;
            }
        ...
        habbo.whisper(Emulator.getTexts().getValue("veto.cmd_pay.received").replace("%amount%", amount + "").replace("%type%", strings[3]).replace("%username%", gameClient.getHabbo().getHabboInfo().getUsername()));
        gameClient.getHabbo().whisper(Emulator.getTexts().getValue("veto.cmd_pay.transferred").replace("%amount%", amount + "").replace("%type%", strings[3]).replace("%username%", habbo.getHabboInfo().getUsername()));
        return true;
    }
}
